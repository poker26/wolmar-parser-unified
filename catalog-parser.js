...
class CatalogParser {
    ...
    parseLotDescription(description) {
        ...
        try {
            ...
            // Извлекаем тираж - только первое число после "тираж"
            const mintageMatch = description.match(/тираж\s+([\d\s,]+?)(?:\s*[,;]|\s+тираж|\s*$)/i);
            if (mintageMatch) {
                const mintageStr = mintageMatch[1].replace(/[\s,]/g, '');
                const mintage = parseInt(mintageStr);
                
                // Дополнительная проверка: тираж не должен содержать точки (как в номерах каталогов)
                // и должен быть в разумных пределах
                if (mintage > 0 && mintage <= 100000000 && !mintageStr.includes('.')) {
                    result.mintage = mintage;
                    console.log(`📊 Извлечен тираж: ${mintage} из "${description}"`);
                } else {
                    console.log(`⚠️ Пропущен неверный тираж: ${mintage} (содержит точку или слишком большой) из "${description}"`);
                }
            }
