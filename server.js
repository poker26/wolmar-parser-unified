...
app.get('/api/logs', (req, res) => {
    try {
        ...
        const requestedLines = Number.parseInt(req.query.lines ?? '50', 10);
        if (!Number.isInteger(requestedLines) || requestedLines < 1 || requestedLines > 1000) {
            return res.status(400).json({ error: 'Параметр lines должен быть целым числом от 1 до 1000' });
        }

        execFile('pm2', ['logs', 'wolmar-parser', '--lines', String(requestedLines), '--nostream'], (error, stdout, stderr) => {
            if (error) {
...
// Обработчик ошибок также должен быть последним middleware.
