...
3. **Git сведён.** Июньский пласт закоммичен на `coin-catalog` (5 тематических коммитов), сверху merge со снимком прода `origin/codex/reconcile-production-20260825` (конфликты: свои фиксы поллера/энума, прод-ссылка «Коллекция» в current.html, объединённый .gitignore). `npm run verify` 4/4.
   **Хвосты:** (а) `git push origin coin-catalog` для merge-коммита ещё не прошёл (блок авто-режима) — запушить; (б) на прод не выкачен `server.js` + `config.production.js` с хардненингом (execFile для pm2-логов, валидация lines, без Authorization в логах) — нужен scp + `pm2 restart wolmar-server`; (в) сабмодуль `wolmar-analytics-clean` содержит незакоммиченный `public/analytics.html` — разбирать в его собственном репозитории.
