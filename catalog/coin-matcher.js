...
// Ту же оценку пишут и наоборот — «Редкая, 2 рубля по Петрову», — и в этом порядке она била
// больнее: у монеты, названной СЛОВОМ («Гривна 1705 года»), своего числа в заголовке нет,
// словесный номинал проверяется последним, и оценка становилась номиналом. Так 14 серебряных
// гривен 1705 года собрались в типе «2 рубля 1705», а альбертусталеры — в «15 рублей 1753».
const REF_PRICE = /(петров|ильин|уздеников|конрос|биткин)[^|.;]{0,15}?\d+(?:[.,]\d+)?\s*руб[а-яё]*|\d+(?:[.,]\d+)?\s*руб[а-яё]*\s+по\s+(петров|ильин|уздеников|конрос|биткин|дьяков|северин|брекке)[а-яё]*/gi;
...
const parseDenomRaw = (t) => {
  ...
  if (fr && +fr[2] !== 0) {
    ...
    // «1/10 унции», «1/2 грамма» — это ВЕС монеты, а не её номинал: у «25 рублей 2023 … 1/10
    // унции. Золото» номинал стоит первым, а дробь описывает вес. Раньше дробь разбиралась
    // первой, лот уходил в иностранную ветку с единицей «унции» и терял страну.
    if (/^(унци|грамм|карат|тро[йи])/.test(unit)) return parseDenomRaw(s.replace(fr[0], " "));
    if (!/^(год|лет|грамм|сохран|экземпл|штук|монет|тысяч|миллион|часть|разн)/.test(unit)) {
      const num = +fr[1] / +fr[2];
...
// «Леопард» 2011 года не находил своего типа вовсе. Ищем в ГОЛОВЕ (до «|»): в прозе продавца
// двор поминают рядом с чужими годами. Совпадение берём левое — у numismat год стоит перед
// двором первым («1 рубль 1813 года, СПБ-ПС»), а отсылки к образцу идут дальше по строке.
const YEAR_MINT = new RegExp("(1[5-9][0-9]{2}|20[0-3][0-9])[ ]*(?:г[.]|года|год|г)?[.,; ]*(?:СПМД|ММД|ЛМД|СПБ|СПМ|ЕМ|ММ|КМ|ТМ|ИМ|НМ|БМ|АМ)(?![А-Яа-яЁё])");
// Полная дата «дд.мм.гггг» — это дата события или жизни человека, а не год монеты: год на монете
// так не пишут. Из-за неё «2 рубля. Физик И.М. Франк - 100 лет со дня рождения (23.10.1908 г.)
...
const NON_THEME = /^(исполнени|proof|пруф|unc(?![а-яё])|bunc|coincard|цветн|буклет|блистер|капсул|футляр|запайк|коробк|конверт|сертификат|позолот|позолоч|нейзильбер|пластик|томпак|титан|мельхиор|биметалл|латун|новинк|распродаж|идеальн|красив|суперблеск|отличн|шикарн|шедевр|редчайш|африка|европа|азия|америка|океания|республик|королевств|федерац|конфедерац|доминион|содружеств|специальн|обычн|улучшенн|качеств|мельхиор|латун|стал[иья]|никел|биметалл|бронз|медно|серебр|золот|платин|паллад|проба|блеск|патина|сохран|состоян|отличн|прекрасн|слаб[еы]|оригинал|подлинн|тираж|монетн|двор[еа]?|биткин|уздеников|петров|ильин|конрос|severin|северин|казаков|гиль(?![а-яё])|брекке|дьяков|краузе|krause|numista|редк|нечаст|немагнит|магнитн|гурт|штемпел|чекан|коллекц|аукцион|лоты|торг|спмд|ммд(?![а-яё])|лмд|петербургск|екатеринбургск|сузунск|аннинск|сестрорецк|колыванск|таврическ|варшавск|кадашевск|изображен|надпис|рельефн|справа|слева|сверху|снизу|вверху|внизу|центре|фоне|окружност|строк|обозначен|стилизован|надчекан|номинал|товарн)/;
...
function pickWithMetal(rows, p, single = 0.65, themed = 0.8, relax = false) {
  ...
  // Названный в лоте металл — ЖЁСТКИЙ признак, когда металл известен у всех кандидатов. У ЦБ один
  // и тот же сюжет выпускают и серебром, и золотом под одним названием, годом и номиналом:
  // «100 рублей. Белка обыкновенная 2023» есть серебряная (5117-0067) и золотая (5217-0049).
  // Металл в этом случае — единственное, что их различает, и лот его называет («Au»). Раньше
  // признак работал только в иностранной ветке, поэтому все 14 золотых «Белок» висели на
  // серебряном типе, а у золотого не было ни одного прохода. Требуем известный металл у ВСЕХ
  // кандидатов: у имперских типов он часто пуст, и там отсев был бы вслепую.
  const wantMetal = lotMetal(p.title);
  if (wantMetal && gated.length > 1 && gated.every((x) => String(x.metal || "").trim())) {
    const f = gated.filter((x) => wantMetal.test(String(x.metal)));
    if (f.length) gated = f;
  }
...
const RU_EXTRA = [
  ...
  ["Тайланд", "Thailand"], ["Бирма", "Myanmar"], ["Цейлон", "Sri Lanka"],
...
const LOT_METAL = [
  // ⚠️ «AU» — не только золото, но и ГРЕЙД (AU50, AU53, AU55, AU58). Без этой оговорки монета
  // «в слабе AU58» считалась золотой, а гейт металла теперь жёсткий — и серебро уехало бы к
  // золотым типам. Вес золота пишут дробью («Au 15,55»), поэтому число с запятой не грейд.
  ...
  if (!CRU) {
    ...
    for (const r of rows) {
      ...
      for (const v of vars) {
        ...
        // Трёхбуквенные имена («Чад», «Гоа») раньше отбрасывались порогом в 4 буквы, и страна
        // терялась целиком. Берём и их, но БЕЗ окончаний: «чад» не должен ловить «чадо».
        if (stem.length >= 4) list.push({ stem: ruNorm(stem), country: r.country, re: wordRe(stem) });
        else if (stem.length === 3) list.push({ stem: ruNorm(stem), country: r.country,
          re: new RegExp("(?<![а-яa-z])" + reEsc(ruNorm(stem)) + "(?![а-яa-z])", "i") });
      }
    }
...
// Страна по единице номинала: считаем по самому каталогу, а не по списку в коде.
const UNIT_COUNTRY = new Map();
async function countryByUnit(pool, unit) {
  if (!unit) return null;
  const key = String(unit).toLowerCase();
  if (!UNIT_COUNTRY.has(key)) {
    const en = enUnit(unit);
    const ru = String(unit).slice(0, 4);
    const rows = (await pool.query(
      `SELECT country, count(*)::int c FROM coin_type
...
async function countryList(pool, title, year = null, unit = null) {
  ...
  // Валюта называет страну. «5 гривен 2023 БОРЩ» и «100 бат 2024. Пластик» страны не называют, но
  // гривна бывает только украинской, а бат — только тайским. Берём такой вывод, лишь когда
  // единица в каталоге практически однозначна: девять типов из десяти у одной страны и их не
  // меньше пяти. Доллар, франк и марку это правило не тронет — они у многих.
  const byUnit = await countryByUnit(pool, unit);
  if (byUnit) return [await eraSection(pool, byUnit, year)];
...
async function matchType(pool, p) {
  ...
  if (d.isRf) {
    ...
    if (p.modMints.length) {
      // У 37 карточек ЦБ двор записан СРАЗУ ДВУМЯ — «ММД и СПМД»: монету печатали на обоих, и это
      // осознанно общая карточка, а не огрех разбора. Прежняя сверка сравнивала строку целиком,
      // поэтому такой тип выбрасывался, как только лот называл двор: «100 рублей. Леопард 2011
      // СПМД» не находил свой тип, хотя без двора находил. Сверяем ПЕРЕСЕЧЕНИЕ дворов.
      const mintsOf = (v) => (String(v || "").match(/(?:СПМД|ММД|ЛМД|СПБ|С-П|М|Л)(?![А-Яа-яЁё])/g) || [])
        .map((x) => MOD_CANON[x] || x);
      rows = rows.filter((x) => {
        if (!x.mint) return true;
        const own = mintsOf(x.mint);
        return own.length ? own.some((mm) => p.modMints.includes(mm))
          : p.modMints.includes(MOD_CANON[String(x.mint).trim()] || x.mint);
      });
