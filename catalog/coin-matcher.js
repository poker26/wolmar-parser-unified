...
// JS \b uses ASCII word semantics and therefore does not delimit Cyrillic mint codes.
const MINT = /(?:^|[^А-ЯЁA-Z0-9])(СПБ|СПМД|СПМ|ММД|ЛМД|ЕМ|ВМ|КМ|ТМ|АМ|ИМ|БМ|СМ|ММ|МД)(?=$|[^А-ЯЁA-Z0-9])/gi;
...
const parseDenom = (t) => {
  ...
  // Проверяем дробь до обычного числа: иначе «1/2 доллара» совпадает с хвостом «2 доллара».
  ...
  const m = s.match(/(\d+(?:[.,]\d+)?)\s*(рубл[а-яё]*|копе[а-яё]*|евро|доллар[а-яё]*|центов?|пенс[а-яё]*|пенни|фунт[а-яё]*|тенге|гривен|гривн[а-яё]*|сум[а-яё]*|песо|юан[а-яё]*|лир[а-яё]*|динар[а-яё]*|дирхам[а-яё]*|драм[а-яё]*|манат[а-яё]*|франк[а-яё]*|сантим[а-яё]*|раппен[а-яё]*|крон[а-яё]*|эре|эйре|вон[а-яё]*|иен[а-яё]*|йен[а-яё]*|рупи[а-яё]*|реал[а-яё]*|талер[а-яё]*|шиллинг[а-яё]*|форинт[а-яё]*|злот[а-яё]*|грош[а-яё]*|стотинк[а-яё]*|бат[а-яё]*|лев[а-яё]*|пфенниг[а-яё]*|марок|марк[аи]|пиастр[а-яё]*|филс|эскудо|афгани)/i);
  if (m) {
