...
const { classifyAuctionRuObservation } = require('./marketplace-observation');
const { extractSlabInfo } = require('../domain/slab-info');

const photoUrl = (offerId) => `/api/coinphoto?k=${offerId}/0.jpg`;
    ...
    const result = await pool.query(
        `INSERT INTO auction_lots
           (source_site,source_category,lot_number,source_url,winning_bid,currency,condition,
            auction_end_date,coin_description,avers_image_url,year,lot_status,category,parsing_method,
         ...
         VALUES ('auction.ru','auction.ru-coins',$1,$2,$3,'RUB',$4,$5,$6,$7,$8,'auction.ru',
                 'auctionru-catalog-poller',$9,$10,$11,$12,$13,$14,$15,$16,$17)
        ...
        [
            ...
            offer.nPhotos > 0 ? photoUrl(offer.offerId) : null,
...
async function main() {
    ...
    if (args[0] === '--test') {
        for (const url of args.slice(1)) {
            const page = parse(await fetchHtml(url));
    ...
    const stat = { sold: 0, active: 0, ended_unsold: 0, candidate: 0, linked: 0, skipped: 0, dead: 0, failed: 0 };

    for (const lot of lots) {
        try {
            const html = await fetchHtml(lot.url);
            if (!html || html.length < 1500) {
                await pool.query("UPDATE auctionru_queue SET status='dead' WHERE offer_id=$1", [lot.offer_id]);
                stat.dead++;
                continue;
            }
            ...
            await pool.query(
                `UPDATE auctionru_queue SET captured=$2,availability=$3,last_price=$4,
                        has_bids=$5,last_checked=now() WHERE offer_id=$1`,
...
module.exports = { parse, saveObservation, main };
