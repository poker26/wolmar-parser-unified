...
async function main() {
    ...
    try {
        ...
        console.log(`🚨 Вы собираетесь поставить ставку ${bidAmount} рублей на лот ${lotUrl}`);
        console.log('🚨 Продолжить? (yes/no)');
        
        // В тестовом режиме автоматически подтверждаем минимальную ставку
        const confirmation = bidAmount === 2 ? 'yes' : 'no';
        console.log(`🚨 Автоматическое подтверждение: ${confirmation} (ставка ${bidAmount} руб.)`);
