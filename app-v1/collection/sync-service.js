    ...
    async loadPhotos(userId, ids) {
        ...
        const entries = await Promise.all(result.rows.map(async (row) => {
            const [originalUrl, displayUrl] = await Promise.all([
                this.storage.downloadUrl(row.object_key_original, this.originalUrlTtlSeconds),
                row.object_key_display
                    ? this.storage.downloadUrl(row.object_key_display, this.originalUrlTtlSeconds)
                    : null,
            ]);
            return [row.id, {
                ...photoFromRow(row),
