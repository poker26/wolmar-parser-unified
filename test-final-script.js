/**
 * Тестовый скрипт для проверки финальной версии оптимизированного обновления
 * Тестируем на аукционе 961 (внутренний номер 2117) - проверяем парсинг всех страниц
 */
...
class TestFinalUpdater {
    ...
    async testAllPagesParsing() {
        console.log('\n🧪 ТЕСТИРУЕМ ПАРСИНГ ВСЕХ СТРАНИЦ АУКЦИОНА 2117:');
    ...
    async run() {
        try {
            ...
            console.log('🧪 ТЕСТИРОВАНИЕ ФИНАЛЬНОЙ ВЕРСИИ СКРИПТА');
            console.log('📋 Тестируем парсинг всех страниц аукциона 2117');
            
            // Тестируем парсинг всех страниц
            const lotsData = await this.testAllPagesParsing();
...
async function main() {
    const tester = new TestFinalUpdater();
    await tester.run();
}
