...
# Принудительное исправление каталога с альтернативным подходом
# Использует другой порт и полностью изолирует каталог

echo "💥 ПРИНУДИТЕЛЬНОЕ ИСПРАВЛЕНИЕ КАТАЛОГА..."
echo "====================================="
...
echo "📊 ЭТАП 1: Полная очистка системы..."
echo "================================="
...
echo ""
echo "📊 ЭТАП 2: Пересоздание основного сервера..."
echo "=========================================="
...
echo ""
echo "📊 ЭТАП 3: Создание каталога на альтернативном порту..."
echo "==================================================="
...
echo ""
echo "📝 Создание server.js для каталога на порту 3002..."
# Создаем server.js на альтернативном порту 3002
cat > server.js << 'EOF'
...
const app = express();
const PORT = 3002; // Альтернативный порт
...
echo "✅ server.js создан для каталога на порту 3002"

echo ""
echo "⚙️ ЭТАП 4: Настройка конфигурации..."
...
echo ""
echo "📦 ЭТАП 5: Установка зависимостей..."
npm install
...
echo ""
echo "🚀 ЭТАП 7: Запуск каталога на порту 3002..."
pm2 start server.js --name "catalog-interface"

if [ $? -eq 0 ]; then
    echo "✅ Каталог монет запущен через PM2 на порту 3002"
else
...
echo ""
echo "📊 ЭТАП 9: Финальная проверка..."
echo "==============================="
...
echo ""
echo "🌐 Тестирование каталога (порт 3002):"
curl -s http://localhost:3002/api/auctions > /dev/null
if [ $? -eq 0 ]; then
    echo "✅ Каталог монет работает на порту 3002"
    echo "🌐 Доступен: http://46.173.19.68:3002"
else
...
echo ""
echo "📋 Первые 3 строки порта 3002:"
...
echo "📚 Каталог монет: http://46.173.19.68:3002"
