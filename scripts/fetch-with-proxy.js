...
async function fetchWithProxy(categoryId = '252', finished = true) {
  console.log('🌐 Using proxy rotation approach...');
  
  // Список бесплатных прокси (можно расширить)
  const proxies = [
    // Добавьте реальные прокси если есть
    // 'http://proxy1:port',
    // 'http://proxy2:port',
  ];
  ...
  console.log('⏳ Using proxy rotation for automated bypass...');
  ...
  try {
    ...
    const filename = `proxy_good${categoryId}_opt${finished ? '2' : '1'}_${timestamp}.html`;
    ...
    if (lotLinks > 0) {
      console.log('🎉 Successfully obtained auction data with proxy!');
...
fetchWithProxy(categoryId, finished);
