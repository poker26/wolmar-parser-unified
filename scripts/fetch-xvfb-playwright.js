...
async function fetchXvfbPlaywright(categoryId = '252', finished = true) {
  console.log('🖥️  Using xvfb + Playwright for Cloudflare bypass...');
  
  const browser = await chromium.launch({
    headless: false, // GUI режим с xvfb
    args: [
      '--no-sandbox',
  ...
  console.log('⏳ Using xvfb + Playwright for automated bypass...');
  ...
  try {
    ...
    while (attempts < 180) { // 3 минуты максимум
      ...
      try {
        ...
        attempts++;
        if (attempts % 30 === 0) {
          console.log(`⏳ Attempt ${attempts}/180 - Still waiting for automatic bypass...`);
        }
      } catch (e) {
    ...
    if (content.includes('Just a moment') || content.includes('Один момент')) {
      console.log('⚠️  Cloudflare challenge still active after 3 minutes');
    ...
    const filename = `xvfb_playwright_good${categoryId}_opt${finished ? '2' : '1'}_${timestamp}.html`;
    ...
    if (lotLinks > 0) {
      console.log('🎉 Successfully obtained auction data with xvfb + Playwright!');
...
fetchXvfbPlaywright(categoryId, finished);
