...
function evidenceFor(proposal) {
    return [
        `bitkin_short_reference:${proposal.shortReference}`,
        `bitkin_entries:${proposal.bitkinEntryIds.join(',')}`,
        'bitkin_short_reference_unique_type',
        'lot_bitkin_type_year_exact',
        'lot_bitkin_type_denomination_exact',
        'lot_bitkin_type_mint_consistent',
    ];
}
...
async function applyOne(client, proposal) {
    ...
    await client.query(
        `UPDATE lot_type_link
         SET type_id = $2,
             match_method = 'bitkin_short_exact_identity',
...
async function main() {
    ...
    let candidates = result.proposals.filter(
        (proposal) => proposal.action === 'short_reference_strict_candidate',
    );
    ...
    if (!options.write) {
        console.log(JSON.stringify({
            ...
            uniqueShortReferences: new Set(candidates.map((proposal) => proposal.shortReference)).size,
