...
объяснимая оценка по точным завершённым проходам. Версия `0.6.4` реализует путь
камера → распознавание → выбор совпадения в каталоге → добавление экземпляра с
приватной фотографией в коллекцию. Определение запускается один раз только после
съёмки обеих сторон; две фотографии отправляются и распознаются совместно.
Каталоговая связь создаётся только при точном совпадении страны, года и номинала;
распознанную монету без карточки можно сохранить как несвязанную.
...
Перед передачей сборки запустите полный выпускной тест:

```powershell
.\scripts\verify-store-release.ps1 -OutputDirectory '..\..\..\outputs'
```

Скрипт проверяет модульные тесты, Android Lint, номер версии, разрешения,
правила хранения данных и подписи APK и AAB. Ключ `-SkipBuild` повторно
проверяет уже собранные файлы.
